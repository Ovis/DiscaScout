...
/// </summary>
public sealed class DiscasScrapeService(IDiscasCategoryCrawler crawler, IDiscasSnapshotStore snapshotStore)
{
    ...
    /// <summary>指定した1カテゴリだけを取得して永続化する</summary>
    public async Task<CategoryScrapeResult> ExecuteCategoryAsync(DiscSourceCategory category, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return CategoryScrapeResult.Success(category, snapshot.TotalCount, snapshot.PageCount, applyResult);
        }
...
/// <summary>1カテゴリのクロールと永続化結果を保持する</summary>
public sealed record CategoryScrapeResult
{
    ...
    internal static CategoryScrapeResult Success(DiscSourceCategory category, int totalCount, int pageCount, SnapshotApplyResult applyResult) => new()
    {
        Category = category,
        IsSuccess = true,
