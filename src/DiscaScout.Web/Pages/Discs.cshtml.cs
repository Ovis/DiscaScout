...
public sealed class DiscsModel(DiscaScoutDbContext dbContext) : PageModel
{
    ...
    public async Task OnGetAsync(CancellationToken cancellationToken)
    {
        ...
        var query = dbContext.Discs.AsNoTracking()
            .Include(x => x.ReviewReasons).Include(x => x.Sources)
            .Include(x => x.ArtistMatches).ThenInclude(x => x.ArtistSetting).AsQueryable();
    ...
    public async Task<IActionResult> OnPostReviewedAsync(long id, string? returnUrl, CancellationToken cancellationToken)
    {
        ...
        SetUndoPayload(disc); disc.NeedsReview = false; disc.LastReviewedAt = TimeProvider.System.GetUtcNow().UtcDateTime;
        dbContext.DiscReviewReasons.RemoveRange(disc.ReviewReasons); await dbContext.SaveChangesAsync(cancellationToken);
        StatusMessage = $"「{disc.Title}」を確認済みにしました"; return RedirectBack(returnUrl);
    }

    public async Task<IActionResult> OnPostRentedAsync(long id, string? returnUrl, CancellationToken cancellationToken)
    {
        var disc = await dbContext.Discs.Include(x => x.ReviewReasons).SingleAsync(x => x.Id == id, cancellationToken);
        SetUndoPayload(disc); disc.IsRented = true; disc.NeedsReview = false; disc.LastReviewedAt = TimeProvider.System.GetUtcNow().UtcDateTime;
        dbContext.DiscReviewReasons.RemoveRange(disc.ReviewReasons); await dbContext.SaveChangesAsync(cancellationToken);
        StatusMessage = $"「{disc.Title}」を借りた状態にしました"; return RedirectBack(returnUrl);
    }
    ...
    public async Task<IActionResult> OnPostReviewPageAsync(List<long> ids, string? returnUrl, CancellationToken cancellationToken)
    {
        ...
        await dbContext.SaveChangesAsync(cancellationToken); UndoPayload = null; StatusMessage = $"現在ページの {discs.Count} 件を確認済みにしました";
        return RedirectBack(returnUrl);
    }
    ...
    public async Task<IActionResult> OnPostUndoAsync(string undoPayload, string? returnUrl, CancellationToken cancellationToken)
    {
        ...
        await dbContext.SaveChangesAsync(cancellationToken); UndoPayload = null; StatusMessage = $"「{disc.Title}」の直前の操作を元に戻しました";
        return RedirectBack(returnUrl);
    }
    ...
    public string? GetRentalCategoryLabel(DateOnly? rentalStartDate)
    {
        ...
        if (elapsedDays < 0) return "近日リリース"; if (elapsedDays <= 90) return "新作"; if (elapsedDays <= 180) return "準新作"; return "旧作";
    }
    ...
    {
        foreach (var term in SplitTerms(TitleSearch)) { var normalized = DiscTextNormalizer.Normalize(term); query = query.Where(x => x.NormalizedTitle.Contains(normalized)); }
        foreach (var term in SplitTerms(ArtistSearch)) { var normalized = DiscTextNormalizer.Normalize(term); query = query.Where(x => x.NormalizedArtist.Contains(normalized)); }
        if (!string.IsNullOrWhiteSpace(Genre))
        {
            // 各CDは大・中・小ジャンルを同時に保持しているため、大ジャンル指定では配下もGenreLargeの一致で含まれる。
    ...
    {
        "rental" => query.OrderByDescending(x => x.RentalStartDate.HasValue).ThenByDescending(x => x.RentalStartDate).ThenBy(x => x.Sources.Where(s => s.IsActive).Select(s => (int?)s.SourceRank).Min() ?? int.MaxValue).ThenByDescending(x => x.LastUpdatedAt),
    ...
    private async Task<IReadOnlyList<GenreGroup>> LoadGenreGroupsAsync(CancellationToken cancellationToken)
    {
        ...
        return genres.GroupBy(x => x.GenreLarge, StringComparer.Ordinal).OrderBy(x => x.Key, StringComparer.Ordinal)
            .Select(largeGroup => new GenreGroup(largeGroup.Key, largeGroup.Where(x => !string.IsNullOrWhiteSpace(x.GenreMiddle)).GroupBy(x => x.GenreMiddle!, StringComparer.Ordinal).OrderBy(x => x.Key, StringComparer.Ordinal)
                .Select(middleGroup => new GenreMiddleGroup(middleGroup.Key, middleGroup.Where(x => !string.IsNullOrWhiteSpace(x.GenreSmall)).Select(x => x.GenreSmall!).Distinct(StringComparer.Ordinal).OrderBy(x => x, StringComparer.Ordinal).ToArray())).ToArray())).ToArray();
    }
    ...
    {
        if (Tab is not ("unchecked" or "pickup" or "all")) Tab = "unchecked"; if (Rental is not ("all" or "rented" or "unrented")) Rental = "all";
        if (Sort is not ("updated" or "rental" or "title" or "artist")) Sort = "updated"; if (PageSize is not (50 or 100 or 200)) PageSize = 50;
        PageNumber = Math.Max(1, PageNumber); TitleSearch = string.IsNullOrWhiteSpace(TitleSearch) ? null : TitleSearch.Trim(); ArtistSearch = string.IsNullOrWhiteSpace(ArtistSearch) ? null : ArtistSearch.Trim(); Genre = string.IsNullOrWhiteSpace(Genre) ? null : Genre.Trim();
    }
    ...
    private static string BuildListUrl(string tab, string? title, string? artist, string? genre, bool excludeMaxi, bool excludeAlbum, string rental, string sort, int size, int page)
    {
        ...
        if (!string.IsNullOrWhiteSpace(title)) values["title"] = title; if (!string.IsNullOrWhiteSpace(artist)) values["artist"] = artist; if (!string.IsNullOrWhiteSpace(genre)) values["genre"] = genre;
        if (excludeMaxi) values["excludeMaxi"] = "true"; if (excludeAlbum) values["excludeAlbum"] = "true"; if (page > 1) values["p"] = page.ToString();
