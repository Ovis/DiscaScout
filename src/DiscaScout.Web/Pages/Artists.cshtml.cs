    ...
    ManualWorkSignal manualWorkSignal) : PageModel
{
    ...
    public async Task<IActionResult> OnPostPreviewAsync(
        long? id, string artist, ArtistMatchType matchType, bool isWatchEnabled, bool collectFullCatalog,
        bool reviewInitialCatalogItems, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> OnPostAddAsync(
        string artist, ArtistMatchType matchType, bool isWatchEnabled, bool collectFullCatalog,
        bool reviewInitialCatalogItems, bool reopenExistingReviewedMatches, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(artist))
        {
            StatusMessage = "アーティスト名を入力してください";
        ...
        bool reviewInitialCatalogItems, bool reopenExistingReviewedMatches, CancellationToken cancellationToken)
    {
        ...
        // 初回取得後に変更しても過去のCatalog取得結果へ遡及しないため、初回完了前だけ設定を変更可能にする。
        ...
        bool reviewInitialCatalogItems, CancellationToken cancellationToken)
    {
        ...
        var matchingDiscs = await dbContext.Discs.AsNoTracking()
            ...
            .Select(x => new { x.Id, x.NeedsReview, x.IsRented }).ToListAsync(cancellationToken);

        HashSet<long> currentMatchIds = [];
        if (id.HasValue)
        {
            currentMatchIds = (await dbContext.DiscArtistMatches.AsNoTracking()
                .Where(x => x.ArtistSettingId == id.Value && x.IsCurrentMatch).Select(x => x.DiscId).ToListAsync(cancellationToken)).ToHashSet();
        }
        ...
        return new ArtistSettingPreview(id, artist.Trim(), matchType, isWatchEnabled, collectFullCatalog, reviewInitialCatalogItems,
    ...
    private async Task LoadAsync(CancellationToken cancellationToken)
    {
        Settings = await dbContext.ArtistSettings.AsNoTracking().OrderBy(x => x.IsArchived).ThenBy(x => x.Artist)
            .Select(x => new ArtistSettingRow(x.Id, x.Artist, x.MatchType, x.IsWatchEnabled, x.CollectFullCatalog,
    ...
    public sealed record ArtistSettingRow(long Id, string Artist, ArtistMatchType MatchType, bool IsWatchEnabled,
        bool CollectFullCatalog, bool ReviewInitialCatalogItems, bool InitialCatalogCollectionCompleted, bool IsArchived,
        int CurrentMatchCount, int ActiveCatalogCount);

    /// <summary>Artist設定の保存前確認で表示する設定値とローカルCDへの影響件数</summary>
    public sealed record ArtistSettingPreview(long? Id, string Artist, ArtistMatchType MatchType, bool IsWatchEnabled,
        bool CollectFullCatalog, bool ReviewInitialCatalogItems, int MatchCount, int ReviewedMatchCount,
        int NewlyMatchedCount, int ReopenCandidateCount);
}
