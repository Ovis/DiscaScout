...
public sealed class ManualWorkStore(DiscaScoutDbContext dbContext)
{
    ...
    public async Task<bool> TryEnqueueCategoryScrapeAsync(ScrapeCategory category, DateTime requestedAt, CancellationToken cancellationToken = default)
    {
        var exists = await dbContext.ManualWorkItems.AnyAsync(
            x => (x.Type == ManualWorkType.FullScrape || (x.Type == ManualWorkType.CategoryScrape && x.Category == category))
                && (x.Status == ManualWorkStatus.Pending || x.Status == ManualWorkStatus.Running),
            cancellationToken);
        if (exists) return false;
    ...
    public async Task<bool> TryEnqueueArtistCatalogAsync(long artistSettingId, DateTime requestedAt, CancellationToken cancellationToken = default)
    {
        var exists = await dbContext.ManualWorkItems.AnyAsync(
    ...
    /// <summary>
    /// 一回限りのArtist全作品取得を同一条件で重複しないようPendingで登録する
    /// </summary>
    /// <param name="artist">DISCASで検索するアーティスト名</param>
    /// <param name="matchType">検索結果のアーティスト表記に対する一致方法</param>
    /// <param name="reviewNewItems">新規発見したCDを未チェックとして保持するか</param>
    /// <param name="requestedAt">要求日時</param>
    /// <returns>新しい要求を登録した場合はtrue、同一条件が既に保留・実行中ならfalse</returns>
    public async Task<bool> TryEnqueueOneShotArtistCatalogAsync(
        string artist,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var exists = await dbContext.ManualWorkItems.AnyAsync(
            x => x.Type == ManualWorkType.OneShotArtistCatalog
                && x.OneShotArtist == trimmedArtist
                && x.OneShotMatchType == matchType
                && x.OneShotReviewNewItems == reviewNewItems
                && (x.Status == ManualWorkStatus.Pending || x.Status == ManualWorkStatus.Running),
            cancellationToken);
        if (exists) return false;
        ...
        dbContext.ManualWorkItems.Add(new ManualWorkItem
        {
            Type = ManualWorkType.OneShotArtistCatalog,
            Status = ManualWorkStatus.Pending,
            OneShotArtist = trimmedArtist,
            OneShotMatchType = matchType,
            OneShotReviewNewItems = reviewNewItems,
            RequestedAt = requestedAt
        });
        await dbContext.SaveChangesAsync(cancellationToken);
        return true;
    ...
    {
        var interrupted = await dbContext.ManualWorkItems.Where(x => x.Status == ManualWorkStatus.Running).ToListAsync(cancellationToken);
        foreach (var item in interrupted)
        {
            // Runningのままでは永久に再開されないため、単一インスタンス起動時に再キューする。
            // 各処理は完全スナップショット反映または冪等な取り込みなので、先頭からの再実行を許容する。
    ...
    /// <summary>最も古いPending要求を取得する</summary>
    public Task<ManualWorkItem?> GetNextPendingAsync(CancellationToken cancellationToken = default) =>
        dbContext.ManualWorkItems.AsNoTracking().Where(x => x.Status == ManualWorkStatus.Pending).OrderBy(x => x.RequestedAt).ThenBy(x => x.Id).FirstOrDefaultAsync(cancellationToken);

    /// <summary>指定要求を実行中へ遷移させる</summary>
    ...
    /// <summary>指定要求を成功終了へ遷移させる</summary>
    ...
    /// <summary>指定要求を失敗終了へ遷移させる</summary>
    public async Task MarkFailedAsync(long id, DateTime completedAt, string? failureReason, CancellationToken cancellationToken = default)
    {
        var item = await dbContext.ManualWorkItems.SingleAsync(x => x.Id == id, cancellationToken);
    ...
    /// <summary>画面表示用に保留・実行中の要求を取得する</summary>
    public async Task<IReadOnlyList<ManualWorkItem>> GetActiveAsync(CancellationToken cancellationToken = default) =>
        await dbContext.ManualWorkItems.AsNoTracking().Where(x => x.Status == ManualWorkStatus.Pending || x.Status == ManualWorkStatus.Running).OrderBy(x => x.RequestedAt).ToListAsync(cancellationToken);

    /// <summary>画面表示用に直近の手動処理要求を新しい順で取得する</summary>
    public async Task<IReadOnlyList<ManualWorkItem>> GetRecentAsync(int count, CancellationToken cancellationToken = default)
    {
        if (count <= 0) throw new ArgumentOutOfRangeException(nameof(count));
        return await dbContext.ManualWorkItems.AsNoTracking().OrderByDescending(x => x.RequestedAt).ThenByDescending(x => x.Id).Take(count).ToListAsync(cancellationToken);
    }
