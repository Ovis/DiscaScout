...
/// <summary>DiscaScoutのSQLite永続化モデルを管理するEF Core DbContext</summary>
public sealed class DiscaScoutDbContext(DbContextOptions<DiscaScoutDbContext> options) : DbContext(options)
{
    public DbSet<Disc> Discs => Set<Disc>(); public DbSet<DiscTrack> DiscTracks => Set<DiscTrack>(); public DbSet<DiscSource> DiscSources => Set<DiscSource>(); public DbSet<DiscReviewReason> DiscReviewReasons => Set<DiscReviewReason>(); public DbSet<DiscChangeHistory> DiscChangeHistory => Set<DiscChangeHistory>(); public DbSet<ArtistSetting> ArtistSettings => Set<ArtistSetting>(); public DbSet<DiscArtistMatch> DiscArtistMatches => Set<DiscArtistMatch>(); public DbSet<DiscArtistCatalog> DiscArtistCatalogs => Set<DiscArtistCatalog>(); public DbSet<ScrapeRun> ScrapeRuns => Set<ScrapeRun>(); public DbSet<ScrapeRetry> ScrapeRetries => Set<ScrapeRetry>(); public DbSet<ScrapeScheduleSettings> ScrapeScheduleSettings => Set<ScrapeScheduleSettings>(); public DbSet<ScrapeGuardSettings> ScrapeGuardSettings => Set<ScrapeGuardSettings>(); public DbSet<ManualWorkItem> ManualWorkItems => Set<ManualWorkItem>(); public DbSet<DiscordNotificationSettings> DiscordNotificationSettings => Set<DiscordNotificationSettings>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        disc.Property(x => x.DiscasId).HasMaxLength(32); disc.Property(x => x.ProductUrl).HasMaxLength(2048); disc.Property(x => x.Title).HasMaxLength(1000); disc.Property(x => x.NormalizedTitle).HasMaxLength(1000); disc.Property(x => x.Artist).HasMaxLength(1000); disc.Property(x => x.NormalizedArtist).HasMaxLength(1000); disc.Property(x => x.GenreLarge).HasMaxLength(200); disc.Property(x => x.GenreMiddle).HasMaxLength(200); disc.Property(x => x.GenreSmall).HasMaxLength(200); disc.Property(x => x.ImageUrl).HasMaxLength(2048); disc.Property(x => x.ImagePath).HasMaxLength(2048); disc.Property(x => x.DetailImageUrl).HasMaxLength(2048);
        disc.HasIndex(x => x.NeedsReview); disc.HasIndex(x => x.IsArchived); disc.HasIndex(x => x.IsRented); disc.HasIndex(x => x.NormalizedTitle); disc.HasIndex(x => x.NormalizedArtist); disc.HasIndex(x => x.GenreLarge); disc.HasIndex(x => new { x.DetailRefreshCompleted, x.DetailFetchedAt }); disc.HasIndex(x => x.RentalHistoryImportedAt);

        var track = modelBuilder.Entity<DiscTrack>(); track.HasKey(x => x.Id); track.Property(x => x.Title).HasMaxLength(1000); track.Property(x => x.Duration).HasMaxLength(100); track.HasIndex(x => new { x.DiscId, x.TrackNumber }).IsUnique(); track.HasOne(x => x.Disc).WithMany(x => x.Tracks).HasForeignKey(x => x.DiscId).OnDelete(DeleteBehavior.Cascade);
